...
test('expectationsGroupUpdate(validInput) should return array of expectations', async () => {
  const provider = {}; // Mock provider
  const client = new OnsiteClient(<any>provider);
  const mockExpectationsResponse = [{ id: 1, name: 'Expectation 1' }];
  client['methods'] = <any>{
    expectationsGroupUpdate: jest.fn().mockResolvedValue(mockExpectationsResponse),
  };
  ...
  expect(client['methods'].expectationsGroupUpdate).toHaveBeenCalledWith({ set: body });
});
